...
class BoxEncoder(nn.Module):
    ...
    def forward(self, bboxes, img_shape):
        """
        Args:
            bboxes: (B, 4) - [x1, y1, x2, y2] 归一化坐标 [0, 1]
            img_shape: (H, W)
        Returns:
...
class OffsetPredictor(nn.Module):
    ...
    def __init__(self, hidden_dim=256, max_offset=0.1):
        super().__init__()
        self.max_offset = max_offset  # 归一化坐标下的最大偏移 (0-1范围)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(64, 4)  # 输出4个偏移值 [Δx1, Δy1, Δx2, Δy2] (归一化)
        )
    ...
    def forward(self, features):
        ...
        Returns:
            offsets: (B, 4) - [Δx1, Δy1, Δx2, Δy2] 归一化单位 [0, 1]
        """
...
class BoxRefinementModule(nn.Module):
    ...
    def forward(self, image_embedding, bbox, img_shape):
        ...
        Args:
            ...
            bbox: (B, 4) - 当前 bbox (归一化坐标 [0, 1])
            img_shape: (H, W)
        Returns:
            offset: (B, 4) - 预测的偏移量 (归一化坐标)
        """
        B = bbox.shape[0]
    ...
    def iterative_refine(self, image_embedding, initial_bbox, img_shape, 
                         max_iter=3, stop_threshold=0.01):
        """
        迭代精炼 bbox
        
        Args:
            image_embedding: (B, 256, 64, 64)
            initial_bbox: (B, 4) - YOLOv8 输出的 bbox (归一化坐标 [0, 1])
            img_shape: (H, W)
            max_iter: 最大迭代次数
            stop_threshold: 早停阈值 (归一化坐标)
        Returns:
        ...
        for iter_idx in range(max_iter):
            ...
            # 确保 x2 > x1, y2 > y1
            new_x2 = torch.maximum(new_x2, new_x1 + 0.01)  # 至少1%的宽度
            new_y2 = torch.maximum(new_y2, new_y1 + 0.01)  # 至少1%的高度
...
def box_iou_loss(pred_boxes, target_boxes):
    ...
    Args:
        pred_boxes: (B, 4) - [x1, y1, x2, y2] 归一化坐标 [0, 1]
        target_boxes: (B, 4) - [x1, y1, x2, y2] 归一化坐标 [0, 1]
...
def visualize_refinement(image, bbox_history, save_path, gt_bbox=None):
    ...
    Args:
        ...
        bbox_history: List[Tensor] - 每次迭代的 bbox (归一化坐标)
        save_path: 保存路径
        gt_bbox: (4,) - Ground truth bbox (归一化坐标，可选)
    """
    ...
    for i, bbox in enumerate(bbox_history):
        ...
        x1, y1, x2, y2 = bbox_np * np.array([W, H, W, H])
        width = x2 - x1
        height = y2 - y1
        ...
        if i < len(bbox_history) - 1:
            next_bbox = bbox_history[i + 1][0].cpu().numpy() * np.array([W, H, W, H])
    ...
    if gt_bbox is not None:
        gt_bbox_np = gt_bbox * np.array([W, H, W, H])
        gt_x1, gt_y1, gt_x2, gt_y2 = gt_bbox_np
...
if __name__ == "__main__":
    ...
    # 归一化坐标
    initial_bbox = torch.tensor([[0.2, 0.2, 0.6, 0.6], [0.4, 0.4, 0.8, 0.8]], dtype=torch.float32).to(device)
    img_shape = (H, W)
    
    # 创建模型
    model = BoxRefinementModule(max_offset=0.1).to(device)
